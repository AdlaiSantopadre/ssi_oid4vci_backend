...
Le API  esistenti “di Veramo” non sono automaticamente un interfaccia diretta con il protocollo http;  
quelle API JavaScript/TypeScript presenti nei pacchetti SDK (@veramo/core e plugin) non diventano automaticamente API in stile Representational state transfer (REST)/http.

L'idea è stata adottare il metodo Sphereon che fornisce una implementazione in cui Veramo è usato come una dipendenza incapsulata. 

## OpenID for Verifiable Credential Issuance

Sphereon è servito come guida per implementare un issuer OID4VCI guidato dagli standard,indipendente da SDK specifici, anche quelli messi a disposizione da Sphereon stesso.
...
Il wallet chiama la route */token* per ottenere l' *access_token* usando il grant; quindi chiama la route */credential* con *Bearer token* e, nei profili reali, una proof legata ad un *nonce* -in crittografia l' uso di numero pseudo casuale utilizzato una sola volta a garantisce la univocità.
...
### Issues affrontate durante l'implementazione
...
OpenID Connect è un protocollo di autenticazione interoperabile costruito sulle basi di OAuth 2.0 per semplificare il modo in cui si verificano le identità presso un server che fornisce la autorizzazione.

Le difficoltà principali nascevano nella configurazione rigorosa dei metadati, nella gestione di issuer stateful, e nella integrazione che necessitava di persistenza e ruoli applicativi.
...
appoggiandoci poi a un database di servizio redatto con la libreria *mongoose.js* per la persistenza di issuer state, DID ,VCstatus e binding applicativo.  
...
*   GET */.well-known/openid-credential-issuer* per i  metadata dell'issuer
*    GET */credential-offer* per la  credential offer ( o pre-authorized code)
*    POST */token* per il token endpoint OAuth (mock)
*    POST */credential* →per la  issuance della VC (mock)
   

Il flusso "Metadata → Offer → Token → Credential" è stato così chiuso permettendo di avere un servizio logico interno al backend senza scendere nel dettaglio degli oggetti.
...
crea una tantum la *did:key issuer*   ed espone le funzioni
...
Con i test si è potuto concludete di avere a disposizione un layer per la gestione del flusso:
...
Rispetto alla assenza di persistenza lo “stato di sessione” non vive più in RAM ma  viene salvato in MongoDB  
...
La risposta al client costruita dal controller *credential-offer.controller.js* include  l'URL dell'issuer *credential_issuer*, il grant di tipo pre-authorized code (indicato anche come state state), e i campi c_nonce e c_nonce_expires_in.  
...
Aggiungiamo  che il pre-authorized_code viene scambiato con un access_token  avendo cura di gestire correttamente il software che analizza la struttura dati, cioè il
...
Il controller valida che il grant_type sia quello corretto e poi recupera la sessione dal database con *getByState(code)*.  
Se la sessione non esiste o è scaduta, risponde con "invalid_grant".  
Altrimenti, se esiste, applica il meccanismo anti-replay: chiama la funzione *consumeState(code)*, che fa un aggiornamento atomico sul documento (solo se lo stato è ancora utilizzabile) e lo porta a status="consumed".

Se l'aggiornamento fallisce, significa che quel codice è già stato usato e quindi viene restituito ancora "invalid_grant".

Diversamente,se l'operazione riesce, il server emette una risposta token json pvvero restituisce un access_token che nel nostro caso coincide col codice senza aggiunte, 
perché stiamo mantenendo il profilo minimale,ripropone c_nonce e la sua scadenza.


Il traguardo acquisito con questo passo è che lo stesso pre-authorized_code non può più essere riutilizzato, effettivamente ciò che è stato verificato con un test, dove una seconda chiamata a */token* aceva come risultato sempre "invalid_grant". Anche per questo meccanismo anti-replay di trova in appendice A1 il codice.

La terza richiesta  è di tipo POST sulla route */credential*.  
Adesso il client chiama l'endpoint passando Authorization: Bearer <access_token> e un body con il credentialSubject (almeno con l'id).
...
Il controller quindi marca la sessione come “issued” per tracciamento (*markIssued*) e restituisce la risposta finale:

 `` format: "jwt\_vc", credential: <JWT> }.``
...
standard di crittografia simmetrica capace di garantire riservatezza e integrità con algoritmo Galois-Counter-Mode e chiavi a 256 bit- così da separare il runtime e backup crittografato *veramo.sqlite.enc*.
...
2) Token (pre-authorized_code -> access_token)

t = Invoke-WebRequest http://localhost:3000/token `
  -Method POST `
  -Headers @{ "Content-Type"="application/x-www-form-urlencoded" } `
  -Body "grant_type=urn:ietf:params:oauth:grant-type:pre-authorized_code & pre-authorized_code=\$code"

$token = ($t.Content | ConvertFrom-Json).access_token
...
4) (Opzionale) Suspend / Revoke sostituendo <VC_ID> con il vcId (hash) salvato in Mongo

Invoke-WebRequest http://localhost:3000/vc-status <VC_ID> `
  -Method PATCH `
